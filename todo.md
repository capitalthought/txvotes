...
- [ ] **create a new repo called vote and make a new version of the site branded as vote.help instead of txvotes.app** (added 2026-05-26 via /todo) — Spin up a national-umbrella version of the platform under a new `capitalthought/vote` repo, branded `vote.help`. Likely overlaps the in-flight multi-state generalization (open PRs #17–20, `USV-*`) and the `usvotes.app` umbrella already reserved for exactly this. Sub-steps: (1) `/repo-setup` a new `capitalthought/vote` repo (fork or fresh copy of this codebase); (2) confirm/register the `vote.help` domain — **purchase, needs Josh approval**; (3) rebrand assets/copy/OG images/logo from "Texas Votes" to the national "Vote" brand; (4) DNS + worker route for `vote.help`; (5) decide repo strategy — is `vote` the new home that supersedes txvotes, or a parallel national front-end over the same KV? Resolve before forking to avoid two diverging codebases.
